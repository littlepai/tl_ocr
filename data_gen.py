...
if __name__ == "__main__":
    def str2int(string):
        try:
            ...
            raise
            return None
    sample_num = str2int(sys.argv[1])
    sample_dir = sys.argv[2]
    if not len(sys.argv) == 3 or not sample_num:
        print("Usage: \n\tpython {} {} <sample_dir>\n".format(sys.argv[0], str2int(sys.argv[1]) if str2int(sys.argv[1]) else "<sample_num>"))
        exit(1)
    
    if os.path.exists(sample_dir):
        shutil.rmtree(sample_dir)
    gen_simulated_img(sample_dir, sample_num)
    
    
    # train_data_dir = "./train"
    # train_data_num = 500000
    # val_data_dir = "./val"
    # val_data_num = 20000
    ...
    # gen_simulated_img(train_data_dir, train_data_num)

    # if os.path.exists(val_data_dir):
    #     shutil.rmtree(val_data_dir)
    # gen_simulated_img(val_data_dir, val_data_num)
